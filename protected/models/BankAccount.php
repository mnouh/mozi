...
class BankAccount extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('userId, bankaccount, routing, name', 'required'),
			array('userId, bankaccount, routing, primary', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>64),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, userId, bankaccount, routing, primary, name', 'safe', 'on'=>'search'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->compare('bankaccount',$this->bankaccount);
		$criteria->compare('routing',$this->routing);
